...
internal class KotlinFileGenerator(private val codeGenerator: CodeGenerator) : FileGenerator {
    override fun generateStateVM(stateModel: StateModel, ksFile: KSFile?) {
        println("Writing file: ${stateModel.name + FILE_NAME_ADDITIONAL}")
        ksFile?.let {
            stateModel.toFileSpec()
                .writeTo(codeGenerator, aggregating = false, originatingKSFiles = listOf(it))
        }
    ...
    private fun StateModel.toFileSpec(): FileSpec {
        ...
        constructors.forEach { handlerType.addFunction(it) }
