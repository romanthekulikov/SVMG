...
class UIStateGenerateVisitor(private val codeGenerator: CodeGenerator, val logger: KSPLogger) :
    KSVisitorVoid() {
    private val stateParser: StateParser = KotlinStateParser(logger)

    override fun visitClassDeclaration(classDeclaration: KSClassDeclaration, data: Unit) {
        logger.warn("Start create state model")
        val stateModel = stateParser.parse(classDeclaration)
        logger.warn("state model created")
        KotlinFileGenerator(codeGenerator).generateStateVM(
            stateModel,
